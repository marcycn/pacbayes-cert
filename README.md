...
results/raw/         per-run artifacts (metrics.json, split_indices.npz, *.pt, environment.json)
...
`results/_stratified_superseded/` holds the runs produced under the rejected
stratified split. They are kept for audit and are not read by `aggregate.py`.
